...
/**
 * Shared "what number is this post in its mission?" helper used by the post-email
 * shim and the RSS feed. Mirrors the website's chronological sort exactly so the
 * numbering is consistent across every surface.
 */
class PostNumber
{
	/**
	 * Per-request cache: $cache[$missionId] = array(post_id => position) when
	 * numbering is on, or FALSE when the mission doesn't use numbering or can't
	 * be loaded.
	 */
	private static $cache = array();

	/**
	 * 1-based chronological position of $postId within $missionId's activated
	 * posts, or NULL when post numbering is not enabled for that mission or the
	 * post can't be located.
	 */
