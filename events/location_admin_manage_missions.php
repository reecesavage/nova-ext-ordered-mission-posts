...
$this->event->listen(['location', 'view', 'data', 'admin', 'manage_missions'], function($event){
  ...
  if(isset($event['data']['missions']['current']))
     ...
     if(isset($event['data']['missions']['completed']))
     {
        ...
        {
          $event['data']['missions']['completed'][$key]['desc'] = $value['desc'] ."<td class='col_100 align_right'><button><a href='#' myaction='count' myid='$key' rel='facebox' class='image'><?=$countId;?>Count</button></td></a>";
        }
     }
     ...
     if(isset($event['data']['missions']['upcoming']))
     {
        ...
        {
          $event['data']['missions']['upcoming'][$key]['desc'] = $value['desc'] ."<td class='col_100 align_right'><button><a href='#' myaction='count' myid='$key' rel='facebox' class='image'><?=$countId;?>Count</button></td></a>";
        }
     }
